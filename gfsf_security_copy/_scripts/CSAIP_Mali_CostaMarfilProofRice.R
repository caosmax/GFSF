...
for(i in seq_along(datatotal)){
    ...
    k[[i]]<- k[[i]] %>% filter(., region==pots[p]) %>% filter(.,year=="2050")
    #reordeno los datos
    ...
    k[[i]]<- k[[i]] %>% group_by(impactparameter,commodity, region, year,Cat) %>% 
    ...
    k[[i]]$Percentage_Change<- ((k[[i]]$`2050`-k[[i]]$`2020`)/k[[i]]$`2020`)*100
...
require(xlsx)
write.xlsx( x = tt,file= paste(copy, "CSAIP.xlsx", sep = ""),
...
for(i in seq_along(dataagg)){
    ...
    if (i== 2){
        ...
        k[[i]]<- k[[i]] %>%
            spread ("year","Val")
        #creo variable para  con CC o NoCC
        k[[i]]<-data.frame(k[[i]],"Cat"=ifelse(k[[i]]$scenario=="NoCC_SSP2","NoCC","CC"))
        #elimino fila innecesarias
        k[[i]]<-k[[i]][,-c(5:7)]
        #creo una variable= cambio porcentual 2020-2050
...
# exportar datos a excel
require(xlsx)
write.xlsx( x = z,file= paste(copy, "CSAIP.xlsx", sep = ""),
...
mdwide<-data.frame(mdwide,"Cat"=ifelse(mdwide$scenario=="NoCC_SSP2","NoCC","CC"))
...
datmed<- datos %>% dplyr::filter(.,scenario=="NoCC_SSP2")
