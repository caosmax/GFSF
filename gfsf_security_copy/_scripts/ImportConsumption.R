...
usda_files$ds<- (usda_files$Stock/usda_files$Consumption)*100 # stock as rate of consumption
usda_files$Con_Im<- (usda_files$Imports/usda_files$Consumption)# consumption as rate of Imports
usda_files$Con_Im[is.infinite(usda_files$Con_Im)]<- 0
usda_files<- na.omit(usda_files)
usda_files$ds[is.infinite(usda_files$ds)]<-0
usda_files$ds[is.na(usda_files$ds)]<-0
usda_files<- filter(usda_files, Con_Im!=0)


### calculating diff of imports
usda_files<- usda_files %>% group_by(Area,add=T) %>% mutate(.,dffm_c = c(NA,diff(log(Con_Im)))) 
usda_files<- na.omit(usda_files)
...
out.countries<- filter(out.countries,Freq<40) # con ausencia de mas de 35 años de datos
