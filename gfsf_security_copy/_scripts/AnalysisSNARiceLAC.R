...
#             
            ...
            ################################# sub region ###############################
            aa<- c("South_America", "Central_America", "caribbean")      #"Northern_America"
            ...
            write.csv(dataSUM,paste("./data/", periododsList[[y]], 
                                    "_StactLACNetworks_",unique(riceTpes[[i]]$Commodity),
                                    ".csv", sep = ""))
            ...
            write.csv(Bcoefstab,paste("./data/", periododsList[[y]], 
                                      "_ResultsLACModelNetworks_",unique(riceTpes[[i]]$Commodity),
                                      ".csv", sep = ""))
            ...
            png(paste("./pic/",periododsList[[y]],
                      unique(riceTpes[[i]]$Commodity),"_LACNetwork" ,".png", sep = ""), 
                width = 800, height = 500)
          op <- par(mar = c(1,0,3,1),mfrow=c(1,1))
            V(iDHHS)$size=(rescale(degi,1,8))*2
            E(iDHHS)$arrow.size <- .2
            plot(iDHHS,vertex.label=V(iDHHS)$vertex.names, layout=layout.fruchterman.reingold,
                  edge.width=E(iDHHS)$Q/2,edge.color="orange",
                 vertex.color="orange", vertex.frame.color="#ffffff",, edge.arrow.size=.4)
...
png(paste("./pic/VariablesModelosNetwork.png", sep = ""), 
    width =800, height = 500)
