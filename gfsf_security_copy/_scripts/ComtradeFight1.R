...
allComReg<- filter(all, !Partner %in% noZonas)
...
# obtener datos para todo el mundo
# obtener los codigos por paise que reporta
aux <- inner_join(x = all, y = reporters, by = c('Reporter' = 'Country'))
all$Reporter_code <- aux[,ncol(aux)]

# obtener los codigos por pais socio
aux <- inner_join(x = all, y = partners, by = c('Partner' = 'Country'))
all$Partner_code <- aux[,ncol(aux)]; rm(aux)
all<- filter(all,!Partner %in% noZonas) %>% filter(., !Reporter %in% noZonas)
...
x=1
vv<- foreach(x=1:nrow(all)) %dopar%{ 
      
      cat(paste0(">>>> Procesed regions ",r1[x],"\n"))    
      

      try(tmp <- get.Comtrade(r = all$Reporter_code[x], 
                                    p = all$Partner_code[x], 
                                    cc = "1006,100610,100630", #"110814,110819, 110820,1006,1102,1108,100610,100630,110813,110813,100590,1201,12110,110100,100111,110290,090111"
                                    ps = "all", 
                                    px = "HS", 
                                    rg = "all", 
                                    fmt = "csv"))
