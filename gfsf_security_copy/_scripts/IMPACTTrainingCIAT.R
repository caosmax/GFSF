# Training IMPACT model version CArlos -----

#Questions: 
# What differences do you note across RCP on prices world? 
# By Kenya, Tanzania and Ethiopia what is the impact on yields, production and area by maize, beans and rice?
# Which RCP revels worse impacts for these countries, use graphs for your explanation. 
...
setwd("C:/Users/CEGONZALEZ/Documents/Jamleck_IMPACTClass")
...
# loading data 
md<-read.csv("Cacao.csv",header=T)

# class data
class(md) # dataframe

# structure data
str(md) # atention with factor variable 
md$impactparameter<- as.character(md$impactparameter)
md$scenario<- as.character(md$scenario)
md$commodity<- as.character(md$commodity)
md$region<- as.character(md$region)
md$productiontype<- as.character(md$productiontype)


#Parameters included
allParameters<- unique(md$impactparameter)
parameters<- c("TYldXAgg -- Total Yield","TAreaXAgg -- Total Area", "QNXAgg -- Net Trade")
pricesW<-  "PWXAgg -- World Prices"
#subset parameters and crops
##Yield, Area, supply
cfiles<- filter(md, impactparameter %in% parameters) # varibles for analysis
cprices<- filter(md, impactparameter==pricesW)   # world prices
cfiles$impactparameter<-revalue(cfiles$impactparameter, c("QNXAgg -- Net Trade"="Net Trade",
                                                        "TAreaXAgg -- Total Area"="Total Area",
                                                        "TYldXAgg -- Total Yield"="Total Yield"))
crops<- c("cbarl","cmaiz","cmill","crice","csorg","cwhea","cocer","ccass","cpota","cswpt","cyams","corat","cbean","cchkp","ccowp","clent",
         "cpigp","copul","ccoco","ccafe","ccott","cteas","cothr","cfodr","cgrss")



# Question1: What differences do you note across RCP on prices world by cereals, rtb, pulses and other crops?------- 
 
cpricesCrops<-  filter(cprices, commodity %in% crops)
cpricesCrops$productiontype<-NULL
cpricesCrops$region<-NULL

cpricesCrops$scenario<-  gsub("Cacao_", '',cpricesCrops$scenario)
cpricesCrops$commodity<- revalue(cpricesCrops$commodity, c("cbarl"="Barley",
                                                           "cmaiz"="Maize",
                                                           "cmill"="Millet",
                                                           "crice"="Rice",
                                                           "csorg"="Sorghum",
...
png(filename= paste(mainDir,"/",subDir,"/","Prices.png",sep=""), 
    width = 12, height = 12, units = 'in', res = 300)


c<-ggplot(data=cpricesCrops, aes(year,Val,group=scenario,color=scenario)) +
      geom_line(linetype="dashed",size=1)+ facet_grid(commodity~., scales = "free") +
      labs(title=paste("World Prices\n All scenarios",sep = ""),y="Prices",x="Year")+
...
cpricesCrops<- cpricesCrops %>% spread(year, Val) %>% mutate(change=((.[,ncol(.)]-.[,7])/.[,7])*100)
cpricesCrops<- cpricesCrops[,c("impactparameter","scenario","commodity", "change" )]

hm.palette <- colorRampPalette(rev(brewer.pal(11, 'Spectral')), space='Lab')  
png(filename= paste(mainDir,"/",subDir,"/","PricesChange.png",sep=""), 
    width = 12, height = 12, units = 'in', res = 300)

n<- ggplot(data =cpricesCrops, aes(commodity,scenario)) + 
      geom_tile(aes(fill = change) , colour = "white")+  #facet_grid(~commodity, drop = T)+
      labs(x=NULL, y=NULL, title=paste("World prices change ",sep = "")) +
...
#By Kenya, Tanzania and Ethiopia what is the impact on yields, production and area by cacao, beans and rice?----

cfiles<- filter(cfiles, impactparameter=="Total Area" | impactparameter=="Total Yield")
cfiles<- filter(cfiles, commodity=="jbean" | commodity=="jrice" | commodity=="jcoco")
cfiles$commodity<- revalue(cfiles$commodity, c("jbean"="Bean",
                                               "jrice"="Rice",
                                               "jcoco"="Cocoa"))

cfiles$scenario<-  gsub("Cacao_", '',cfiles$scenario)
cfiles<- filter(cfiles, year==2050)
cfiles$productiontype<- NULL
cfiles<- cfiles %>% spread(scenario, Val)
cfiles$change_26<- ((cfiles$IPSL_2.6-cfiles$NoCC)/cfiles$NoCC)*100
cfiles$change_45<- ((cfiles$IPSL_4.5-cfiles$NoCC)/cfiles$NoCC)*100
cfiles$change_85<- ((cfiles$IPSL_8.5-cfiles$NoCC)/cfiles$NoCC)*100
cfiles<- cfiles[,c("impactparameter","commodity","region","change_26","change_45","change_85")]
cf<- cfiles
cf<- cf %>% gather(sce, Val,4:ncol(cf))

cf<- cf[grep(pattern = "LAC",x = cf$region, ignore.case = T),]

png(filename= paste(mainDir,"/",subDir,"/","AreaYieldChange.png",sep=""), 
    width = 12, height = 12, units = 'in', res = 300)

n<- ggplot(data =cf, aes(sce,region)) + 
      geom_tile(aes(fill = Val) , colour = "white")+  facet_grid(impactparameter~commodity, drop = T)+
      labs(x=NULL, y=NULL, title=paste("World prices change ",sep = "")) +
      scale_fill_gradientn(colours = hm.palette(100))+ labs(x = "",y = "") +  #  theme_grey() 
      scale_x_discrete(expand = c(0, 0)) + scale_y_discrete(expand = c(0, 0)) + coord_equal()+ 
      theme(axis.text.x = element_text(angle = 90, hjust = 1))+
