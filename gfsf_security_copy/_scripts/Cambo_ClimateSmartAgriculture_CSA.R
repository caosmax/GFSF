...
crop.in<- c(  "Cattle","Poultry","Maize","Rice", "Cassava","Fruits" ,
              "Vegetable", "Sugarcane", "Pulses" )

 
# Dataframe para hacer analisis por systmas de produccion
phi_sys<- phi 
qq<- unique(phi_sys$commodity)
qq<- c("Cattle","Poultry","Maize","Rice", "Cassava","Fruits" ,
       "Vegetable", "Sugarcane", "Pulses")
...
cultivations<-  c(  "Maize","Rice", "Cassava","Fruits","Vegetable", "Sugarcane", "Pulses" )
cultivationsTrade<- c(crop.in)

animals<- c( "Cattle","Poultry" )
other<- c("-")
...
require(xlsx)
write.xlsx( x = z,file= paste(copy, "CAM3.3.xlsx", sep = ""),
...
for (i in seq_along(datatotal)){
      ...
      png(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",datatotal[i],"CAM3.3.png",sep=""), 
...
for (i in seq_along(dataagg)){
      ...
      png(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataagg[i],"CAM3.3.png",sep=""), 
          width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(dataanimal)){
      ...
      png(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataanimal[i],"CAM3.3.png",sep=""), 
          width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(dataespecial)){
      ...
      png(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataespecial[i],"CAM3.3.png",sep=""), 
          width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(datasys)){
      ...
      png(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",datasys[i],"CAM3.3.png",sep=""), 
          width = 10, height = 10, units = 'in', res = 300)
...
write.csv(mdwide_tan,paste(copy,"mdwideCAMTotal3.3.csv", sep = ""), row.names = FALSE)
...
write.csv(tznet,paste(copy,"CambiosRelativosCAMTOTAL3.3.csv", sep = ""), row.names = FALSE)
...
##Copia de seguridad cambios relativos y vectores de desempeño
write.csv(tanz,paste(copy,"completeNetTrade3.3.csv", sep = ""), row.names = FALSE)
...
for (i in 1:length(cultivationsTrade)){
      ...
      if(length(which(tanz$commodity==cultivationsTrade[[i]]))>0){
            ...
            z <- z[,c("impactparameter","scenario", "commodity", "region", "Cat", "impacto")] %>% 
                  spread("scenario","impacto")
            if(x>= 3){
                  z$trend <- names(x)[1]
            }
            c[[i]] <- z
...
write.csv(c, paste(copy, "NettradeCAMTOTAL3.3.csv", sep = ""),row.names = FALSE)
...
##Calculo de la media
tznet2_t$mean <- rowMeans(x=tznet2_t[,6:11], na.rm=TRUE)
# c$NoCC_mean <- c[,11]
...
write.csv(x =tznet2_t, file =paste(copy,"areayieldCAM3.3.csv",sep = ""))
...
write.csv(csa, paste(copy, "CAMAreaYield3.3.csv", sep = ""), row.names = FALSE)


#grafico Area de cultivo
png(filename=paste(grd,"AreaCommoditiesCAM3.3.png", sep=""), width = 10, height = 10, units = 'in', res =800)
...
png(filename=paste(grd,"Numer Animales CAM3.3.png", sep=""), width = 10, height = 10, units = 'in', res =800)
