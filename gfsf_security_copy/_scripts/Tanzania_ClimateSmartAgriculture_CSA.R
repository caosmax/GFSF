...
md<-read.csv("CSA_TANZ2.csv",header=T)
phi<- md
...
phi$commodity<-revalue(phi$commodity, c( "cbeef"="Cattle",
                                         "jbeef"="Cattle",
                                         "cpoul"="Chicken",
                                         "jpoul"="Chicken",
                                         "ceggs"="Eggs",
                                         ...
                                         "cbana"="Banana",
                                         "jbana"="Banana",
                                         "csfol"="Sunflower Oil_Processed Oils",
                                         "jsfol"="Sunflower Oil_Processed Oils",
                                         "csfml"="Sunflower_Oilmeals",
                                         "jsfml"="Sunflower_Oilmeals",
                                         "csunf_oil"="Sunflower Oil Agg",
                                         "jsunf_oil"="Sunflower Oil Agg",
                                         "csfnt"="Sunflower_Non-traded Oilseed",
                                         "jsfnt"="Sunflower_Non-traded Oilseed",
                                         "csnfl"="Sunflower_Traded Oilseeds",
                                         "jsnfl"="Sunflower_Traded Oilseeds",
                                         "jsfolt"= "Sunflower Oil_Processed Oils"))
...
cultivations<-  c("Maize","Rice","Sorgum","Cassava","Beans","Banana","Sunflower_Traded Oilseeds",
                   "Sunflower_Non-traded Oilseed", "Sunflower Oil_Processed Oils","Sunflower_Oilmeals","Meat","Sunflower Oil Agg")
animals<- c("Meat","Eggs","Cattle", "Chicken")
other<- c("-")
...
require(xlsx)
write.xlsx( x = z,file= paste(copy, "TanzaniaVersion2.xlsx", sep = ""),
...
for (i in seq_along(datatotal)){
    ...
    tiff(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",datatotal[i],"TanzaniaV2.tiff",sep=""), 
...
for (i in seq_along(dataagg)){
    ...
    tiff(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataagg[i],"TanzaniaV2.tiff",sep=""), 
         width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(dataanimal)){
    ...
    tiff(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataanimal[i],"TanzaniaV2.tiff",sep=""), 
         width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(dataespecial)){
    ...
    tiff(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",dataespecial[i],"TanzaniaV2.tiff",sep=""), 
         width = 10, height = 10, units = 'in', res = 300)
...
for (i in seq_along(datasys)){
    ...
    tiff(filename=paste("C:/Users/CEGONZALEZ/Documents/CSA/",datasys[i],"TanzaniaV2.tiff",sep=""), 
         width = 10, height = 10, units = 'in', res = 300)
...
write.csv(mdwide_tan,paste(copy,"mdwideTanzaniaTotal.csv", sep = ""), row.names = FALSE)
...
nn<-  which(tznet$X2050<0 & tznet$X2020<0) # net trade negativo  importador neto
pn<-  which(tznet$X2050>0 & tznet$X2020<0) # impacto positivo inicia como importador termina como exportador
np<-  which(tznet$X2050<0 & tznet$X2020>0) # impacto negativo inicia como exportador termina como importador
pp<-  which(tznet$X2050>0 & tznet$X2020>0) # net trade positivo  exportador neto
...
for(j in 1:nrow(tznet)){
  ...
  if(j %in% hard){
    tznet$impacto[j] <- abs(tznet$X2050[j] - tznet$X2020[j])/max(abs(tznet$X2050[j]), abs(tznet$X2020[j]), na.rm=TRUE) * 100
  } else {  }
  
}

##Copia de seguridad cambios relativos y vectores de desempeño
write.csv(tznet,paste(copy,"CambiosRelativosTanzaniaTOTALVer2.csv", sep = ""), row.names = FALSE)

# copia
tanz<- tznet
tanz$trend[bad] <- "Negative"
tanz$trend[good] <- "Positive"

c <- list()

for (i in 1:length(cultivations)){
  
  if(length(which(tanz$commodity==cultivations[[i]]))>0){
    z <- tanz[which(tanz$commodity==cultivations[[i]]),]
    ...
    cat(paste('Commodity:', cultivations[i], 'does not have data\n', sep=''))
...
write.csv(c, paste(copy, "NettradeTOTALVer2.csv", sep = ""),row.names = FALSE)

#Area  y rendimientos---------------------------------------
tznet2<- mdwide_tan

tznet2$Percentage_Change<- ((tznet2$X2050-tznet2$X2020)/tznet2$X2020)*100

##Eliminamos las columnas que no necesitamos 
tznet2<- data.frame(tznet2[1:4], tznet2[,36:37])
...
mdwide_tzaCSA<- mdwide_tzaCSA[,c("impactparameter", "scenario","commodity","region","X2020","X2025","X2030", "X2035","X2040","X2045", "X2050", "Cat" )]
...
write.csv(csa, paste(copy, "CSAAreaYieldV2.csv", sep = ""), row.names = FALSE)


#grafico Area de cultivo
tiff(filename=paste(grd,"AreaCommoditiesTanzaniaV2.tiff", sep=""), width = 10, height = 10, units = 'in', res =800)
...
tiff(filename=paste(grd,"Numer Animales TanzaniaV2.tiff", sep=""), width = 10, height = 10, units = 'in', res =800)
