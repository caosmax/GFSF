...
# 
# 
# land<- cc[grep("Land Cover", cc$VARIABLE),]
# q<- cc[grep("Production", cc$VARIABLE),]
# policy<- cc[grep("Policy Cost", cc$VARIABLE),]
# exp<- cc[grep("Export", cc$VARIABLE),]
# imp<- cc[grep("Import", cc$VARIABLE),]
# 
# cons$VARIABLE<- as.character(cons$VARIABLE)
# unique(cons$VARIABLE)
# #### list of all databases 
# ### unit share= Yield
# ### MT/yr= consumption, production(q), exports, imports
# ### billion dolars= policy, 
# ### US$2005/t= price
# ### million Ha/yr= land
# # Listall<- list(y=yield,p=price,f=food, c=cons,l=land, q=q, po=policy, x=exp, m=imp)
# 
# ## anidando base de datos objectos lista en un lista mayor
# tones<- list(consumo=cons,production=q, export=exp, import=imp)
# # rend<- list(yield=yield)
# # billion<-list(policy=policy)
# # precio<- list(pirce=price)
# # tierra<- list(land=land)
# 
# 
# #### production, consumption, exports and imports--------------
# tones$consumo$VARIABLE<- as.character(tones$consumo$VARIABLE)
# vconsumo<- unique(tones$consumo$VARIABLE)
# i=1
