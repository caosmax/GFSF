...
dom<- "//dapadfs/workspace_cluster_6/Socioeconomia/GF_and_SF/RepDom/"
...
rep<- dplyr::filter(mdsubcrop,region=="LAC-Dominican Republic")
write.csv(rep, paste(dom,"IMPACTResults.csv", sep = ""))
...
repCrop<- dplyr::filter(cfiles,FPU=="DOM_DOM")
write.csv(repCrop, paste(dom,"CropModelingResults.csv", sep = ""))
...
cfilesNEt$sys<- plyr::revalue(cfilesNEt$sys, c("IRRI"="Irrigador","RA"="Secano"))
...
repCropPix<- dplyr::filter(cfiles,FPU=="DOM_DOM")
write.csv(repCrop, paste(dom,"CropModelingPIXResults.csv", sep = ""))
...
repCropSui<- dplyr::filter(sui,Region=="DOM")
write.csv(repCropSui, paste(dom,"CropSui.csv", sep = ""))
...
#             filter(.,Crop!="Yam")
      ...
      pic3<- ggplot(data=suifiles, aes(x=Crop, y=val,fill=Crop)) + 
            ...
            labs(x="Cultivos",y="Cambio porcentual en idoneidad")+
            theme(legend.position = "none")+ 
...
#             theme(strip.text.y = element_text(angle = 0,size = 30, face = "bold.italic"))+
            labs(fill=labs2)+ theme(aspect.ratio = 1)+
            theme(#legend.text = element_text(size=11),
                  axis.text =(element_text(face = "bold.italic", size = 30)),
