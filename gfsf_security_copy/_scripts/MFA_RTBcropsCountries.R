...
for(i in 1:length(jdata)){
      ...
      #       if(i==3){
      #             jdata[[i]]<- left_join(jdata[[i]], countries,by=("Regions"))
      #             jdata[[i]]<- jdata[[i]][,c("Scenarios","Commodity","Sys","Year","Val","parameter","IMPACT.Name")]
      #             colnames(jdata[[i]])<- c("Scenarios","Commodity","Sys","Year","Val", "parameter","Regions")
      #             jdata[[i]]<-jdata[[i]][,c("Scenarios","Commodity","Regions","Sys","Year","Val","parameter")] 
      #             
      #             jdata[[i]]<- filter(jdata[[i]], Regions %in% r4 ) 
      #             jdata[[i]]<- filter(jdata[[i]], Commodity %in% jrtb)
...
for(s in 1:length(ad_crbind)){ 
      ...
      lapply(1:length(ad_crops), function(c){
            ...
            if(!dir.exists(paths = paste(wk_dir, "/","Countries","/",crop, sep = ""))){ dir.create(path = paste(wk_dir, "/","Countries","/",crop, sep = "")) }
            
            # Creando data frame 
            dff<- ad_crops[[c]]
            # dff<- dff[,-c(6,10,13,19,22)]
            ...
            dff <- dff[,names(which(miss <= 33))]
            
            dff[is.na(dff)]<- 0
            dff <- Filter(function(x) sd(x) != 0, dff) 
            
            #estandarizar
            dff<- scale(dff)*sqrt(145/144)
            
            # Correlation matrix 
            if(!file.exists(paste(wk_dir, "/","Countries","/",crop,"/",sce,"_corrMatrix.png", sep = ""))){ 
                  
                  M<- cor(dff)
                  png(file = paste(wk_dir, "/", "Countries","/",crop,"/",sce,"_corrMatrix.png", sep = ""), height = 8, width = 8, units = "in", res = 300) 
                  corrplot.mixed(M) 
                  dev.off() 
            } 
            ...
            #             # Distance
            #             distance <- get_dist(dff,stand = TRUE, method = "kendall")
            #             fviz_dist(distance,order = T,show_labels = T)
