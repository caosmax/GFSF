...
### Crear archivo madre
cfiles <- read.csv("lamp_native_20180817-143731.csv")
...
### Eliminar MEG4 ####
mod<-c("ADAGE","Phoenix_6LA","GCAM_LAMP")
food<- food %>% dplyr::filter(Model%in%mod)
...
# write.csv(proof,"./Results/.csv")

### food_prod_value ####
## CC impacts ref_nocfe_notx - sce_nocfe_notx
food_prod_value_imp<- food_prod_value %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
#Filtrar por modelo
fpv_imp<- fpv_imp %>%filter(Region=="COL")
...
gf1<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Food production value", x="pct")
plot(gf1)

### diet_adeq ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- diet_adeq %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
imp2<- diet_adeq %>% dplyr::filter(Scenario%in%ref_nocfe_notx)
tot_imp<-rbind(imp, imp2)
#Filtrar por modelo
tot_imp<- tot_imp %>%filter(Region=="COL")
# Calcular variación % 2010 a 2050
tot_imp$var<- (tot_imp$`2050` - tot_imp$`2010`)
# Gráfico de densidad
proof<- tot_imp %>%  select(Model,Region,Scenario,var)

gf3<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Dietary energy supply adequacy", x="pct")
plot(gf3)

### food_exp_share ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- food_exp_share %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
gf5<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Food Expenditure Share of Income", x="pct")
plot(gf5)

### share_staples ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- share_staples %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
gf7<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Share of dietary energy supply", x="pct")
plot(gf7)

### protein_total ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- protein_total %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
gf9<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Protein total", x="pct")
plot(gf9)

### protein_animal ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- protein_animal %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
gf11<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="Protein animal", x="pct")
plot(gf11)

### gdp_cap_ppp ####
## CC impacts ref_nocfe_nopol - sce_nocfe_notx
imp<- gdp_cap_ppp %>% dplyr::filter(Scenario%in%sce_nocfe_notx)
...
gf13<- ggplot(proof, aes(var)) + geom_density(aes(color=Model),alpha = 0.5) +
  labs(title="GDP per capita PPP", x="pct")
plot(gf13)

### Combinar gráficos ####
png(filename = paste("./pic/","cc_imp_COL.png", sep=""), width = 13, height = 6,
    units = 'in', res = 400)

gg <- ggarrange(gf1,gf3,gf5,gf7,gf9,gf11, ncol = 3, nrow = 2,common.legend = F,
                legend ="bottom")
plot(gg)
dev.off()
