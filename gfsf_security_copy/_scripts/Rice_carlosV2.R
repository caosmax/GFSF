...
#   tempcolor<- unique(rjoin$zone)
  ...
  color<- rjoin %>% filter(., mean>0.8) %>% select("Reporter","Partner" , "color") 
  ...
  chordDiagram(x = rdata[rdata$mean>0.8,],col=color$color,  transparency = 0.25, #col=color$color #[rdata$mean>0.7,]
               directional = 1,
