...
# Scenarios productivity and water improved 
...
# i=1
# p=1
cdata<- lapply(cdata, function(x){
      x$Scenarios<-  gsub("'",'', x$Scenarios)
      s<-unique(x$Scenarios)
      x$Commodity<-  gsub("'", '', x$Commodity)
      x$Regions<-  gsub("'", '', x$Regions)
      x$Year<-  gsub("'",'', x$Year)
      
      x$Scenarios<- as.character(x$Scenarios)
      x$Commodity<- as.character(x$Commodity)
      x$Regions<- as.character(x$Regions) 
      x$Year<- as.numeric(x$Year)
      x<- filter(x, !Commodity %in% nocrops )
      
      # Select regiones 
      x<-  x[grep(pattern= "EAP",x =  x$Regions, ignore.case = T),]
      # x<- filter(x, Scenarios %in% s)
      x<- filter(x, Regions %in% eap) 
      x<-  x%>% spread(Year, Val) 
      
      x$Scenarios<- plyr::revalue(x$Scenarios, c("SSP2-HGEM2"="REF_HGEM",
                                                 "SSP2-HGEM-HiNARS2"="HIGH+NARS",
                                                 "SSP2-HGEM-HiREFF2"="HIGH+RE",
                                                 "SSP2-HGEM-HiYld2"="HIGH",
                                                 "SSP2-HGEM-IRREXP2"="IX",
                                                 "SSP2-HGEM-IRREXP-WUE2"="IX+WUE",
                                                 "SSP2-HGEM-LoYld2"="MED",
                                                 "SSP2-HGEM-MMEFF2"="RMM",
                                                 "SSP2-HGEM-Pangloss2"="COMP",
                                                 "SSP2-HGEM-RegYld2"="REGION",
                                                 "SSP2-HGEM-SWHC2"="ISW",
                                                 "SSP2-IPSL2"="REF_IPSL",
                                                 "SSP2-IPSL-IRREXP2"="IX",
                                                 "SSP2-IPSL-IRREXP-WUE2"="IX+WUE",
                                                 "SSP2-IPSL-Pangloss2"="COMP",
                                                 "SSP2-IPSL-SWHC2"="ISW",
                                                 "SSP2-NoCC"="REF_NoCC",
      ...
      #loop para crear plots de tendencia
      for(p in 1:length(parameter)){
            
            # Creating directories 
            wk_dir <- "//dapadfs/workspace_cluster_6/Socioeconomia/GF_and_SF/USAIDForGFSF/ASEAN" 
            if(!dir.exists(paths = paste(wk_dir, "/",parameter[p], sep = ""))){ dir.create(path = paste(wk_dir, "/", parameter[p], sep = "")) }
            
            xy<- filter(x, parameter==parameter[p])
            n1<-match("2010",names(xy))
            
            # Trends 
            ## solo escenarios sin NoCC
            datmin<- filter(xy,!Scenarios %in% nocc)
            ...
            datmed<- filter(xy, Scenarios %in% nocc)
            ...
            datmax<- filter(xy, !Scenarios %in% nocc)
            ...
            tiff(filename=paste(wk_dir, "/", parameter[p],"/", parameter[p],"_Trends.tiff", sep = ""), 
                 width = 12, height = 12, units = 'in', res = 100)
            
            pic<- ggplot(data=datost,aes(time,datmed,group=Commodity,color=Commodity)) + facet_wrap(~Regions, scales = "free")+
                  geom_line(linetype="dashed",size=1)+ 
                  geom_ribbon(aes(ymin=datmin,ymax=datmax,fill=Commodity,colour=Commodity,linetype=NA),alpha=0.1) +
                  labs(y=parameter[p],x="Year", title= parameter[p])+ 
                  # coord_equal()+ 
                  ...
                  theme(legend.position="bottom")+ scale_x_continuous( breaks=seq(2010,2050,5))
            ...
            # Heat    
            xy$change<- ((xy$`2050`-xy$`2010`)/xy$`2010`)*100
            xy<-xy[,c("Scenarios", "Commodity", "Regions", "parameter", "change" )]
            
            
            xy$change[is.na(xy$change)==TRUE]<- 0
            ### Eliminar ceros
            xy<-xy[!(xy$change==0),]
            ...
            # organizando orden paises
            cfilesNEt$Regions <- factor(cfilesNEt$Regions, levels = c("EAP","EAP-Cambodia","EAP-Indonesia",
                                                                      "EAP-Malaysia","EAP-Myanmar","EAP-Laos","EAP-Philippines","EAP-Thailand","EAP-Vietnam"))
            
            #Organizando los escenarios
            cfilesNEt$Scenarios <- factor(cfilesNEt$Scenarios, 
                                          levels = c("HIGH+NARS","HIGH+RE","HIGH","REGION","RMM", "COMP","IX+WUE","IX","MED","ISW","REF_HGEM","REF_IPSL","REF_NoCC",
                                                     "NoCC_IX+WUE","NoCC_IX","NoCC_COMP","NoCC_ISW"))
            
            totalcrops<-length(unique(cfilesNEt$Commodity))
            nfilas<- round(totalcrops/5)
            
            
            tiff(filename=paste(wk_dir, "/", parameter[p],"/", parameter[p],"_HeapMap.tiff", sep = ""), 
                 width = 14, height = 14, units = 'in', res = 100)
            
            pic1<- ggplot(data =cfilesNEt, aes(Scenarios, Regions)) + 
                  geom_tile(aes(fill = change), colour = "white")+  facet_wrap(~Commodity,nrow = nfilas,ncol=7,drop = T)+
                  labs(x=NULL, y=NULL,title= parameter[p], fill="Percentage\nChange(%)" ) +
            ...
            cat(paste("Terminado  grafico para el parametro ", parameter[p], " it was completed\n", sep=""))     
            
      }

      return(x)
})
rm(cdata,cfiles, cfOut,datmax,datmin,datmed,datost,extremos,x,xy,cfilesNEt)
...
cdata<- lapply(cdata, function(x){
      ...
      x$Sys<- plyr::revalue(x$Sys,c("air"="Irrigated", "arf"="Rainfed"))
      
      # vector con los parametros
      parameter<-unique(x$parameter)
      
      # p=2 solo para realizar pruebas
      ...
      for(p in 1:length(parameter)){
            ...
            #filter por parametro
            xy<- filter(x, parameter==parameter[p])
            
            #corto los datos por sistema
            gfiles<- xy %>% split(xy$Sys) 
            
            sysFiles<- lapply(gfiles, function(s){
                  
                  
                  # Trends 
                  ## solo escenarios sin NoCC
                  n1<- match("2010",names(s))
                  
                  datmin<- filter(s,!Scenarios %in% nocc)
                  
                  datmin<- aggregate(datmin[,n1:ncol(datmin)],by=list(datmin$Regions,datmin$Commodity, datmin$Sys),FUN=min)
                  datmin<- datmin %>% gather("time","datmin", 4:ncol(datmin))
                  names(datmin)<-c("Regions","Commodity","Sys","time","datmin")
                  
                  ## valor medio solo para NoCC, escenarios
                  datmed<- filter(s, Scenarios %in% nocc)
                  datmed<- aggregate(datmed[,n1:ncol(datmed)],by=list(datmed$Regions,datmed$Commodity, datmed$Sys),FUN=median)
                  datmed<- datmed %>% gather("time","datmed", 4:ncol(datmed))
                  names(datmed)<-c("Regions","Commodity","Sys","time","datmed")
                  
                  datmax<- filter(s, !Scenarios %in% nocc)
                  datmax<-aggregate(datmax[,n1:ncol(datmax)],by=list(datmax$Regions,datmax$Commodity, datmax$Sys),FUN=max)
                  datmax<- datmax %>% gather("time","datmax", 4:ncol(datmax))
                  names(datmax)<-c("Regions","Commodity","Sys","time","datmax")
                  ...
                  tiff(filename=paste(wk_dir, "/", parameter[p],"/", parameter[p],"_",unique(datost$Sys),"_Trends.tiff", sep = ""), 
                       width = 10, height = 8, units = 'in', res = 100)
                  
                  pic<- ggplot(data=datost,aes(time,datmed,group=Commodity,color=Commodity)) + facet_wrap(~Regions, scales = "free")+
                        geom_line(linetype="dashed",size=1)+ 
                        geom_ribbon(aes(ymin=datmin,ymax=datmax,fill=Commodity,colour=Commodity,linetype=NA),alpha=0.1) +
                        labs(y=parameter[p],x="Year", title= paste(parameter[p], "\n ", unique(datost$Sys), sep = ""))+ 
                        # coord_equal()+ 
                        theme(axis.text.x = element_text(angle = 90, hjust = 1))+
                        theme_grey() + 
                        theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 8))+
                  ...
                  # Heat    
                  s$change<- ((s$`2050`-s$`2010`)/s$`2010`)*100
                  s<-s[,c("Scenarios", "Commodity", "Regions","Sys", "parameter", "change" )]
                  
                  s$change[is.na(s$change)==TRUE]<- 0
                  ### Eliminar ceros
                  s<-s[!(s$change==0),]
                  ...
                  totalcrops<-length(unique(cfilesNEt$Commodity))
                  nfilas<- round(totalcrops/7)
                  
                  
                  
                  tiff(filename=paste(wk_dir, "/", parameter[p],"/", parameter[p],"_",unique(cfilesNEt$Sys),"_HeapMap.tiff", sep = ""), 
                       width = 14, height = 10, units = 'in', res = 100)
                  
                  pic1<- ggplot(data =cfilesNEt, aes(Scenarios, Regions)) + 
                        geom_tile(aes(fill = change), colour = "white")+  facet_wrap(~Commodity,nrow = nfilas,ncol=8,drop = T)+
                        labs(x=NULL, y=NULL,title=paste(parameter[p], "\n",unique(cfilesNEt$Sys), sep = ""), fill="Percentage\nChange(%)" ) +
                        scale_fill_gradientn(colours = hm.palette(100),limits=c(-lit,lit))+   
                        #coord_equal()+ 
                        theme(axis.text.x = element_text(angle = 90, hjust = 1))+
                        theme(axis.text.x = element_text(angle = 90, hjust = 1))+
...
water <- arrangeGrob(w1, w2, nrow=2) #generates g
ggsave(file=paste(wk_dir, "/", "Water","_HeapMap.png",sep = ""), water, width=16, height=16, units='in') #saves g
