...
db_time$Reporter[grep(pattern = "China, Hong Kong SAR", x = db_time$Reporter, fixed = TRUE)] <- 'Hong Kong SAR'
...
db_time$Partner[grep(pattern = "China, Hong Kong SAR", x = db_time$Partner, fixed = TRUE)] <- 'Hong Kong SAR'
...
#   tempcolor<- unique(rjoin$zone)
  ...
  color<- rjoin %>% filter(., mean>0.85) %>% select("Reporter","Partner" , "color") 
  ...
  # Save plot
  png(paste('C:/Users/CEGONZALEZ/Documents/cassava/copyData/Dried_circos3_', periodList[i],'.png', sep = ''), width = 10, height = 10, units = 'in', res = 300)
  
  chordDiagram(x = rdata[rdata$mean>0.85,],col=color$color,  transparency = 0.25, #col=color$color #[rdata$mean>0.7,]
               directional = 1,
