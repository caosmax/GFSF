# Cassava starch export flows - Preparing for CIAT-blog
...
db_dir <- './_data/_cassava_starch_data/' # Using mean as summary measure
...
db_time <- lapply(1:length(db_periods), function(i){
  ...
  z$X <- NULL
  colnames(z) <- c('Reporter', 'Partner', 'Mean') # Mean
...
real_exports <- db_time[db_time$Mean > 0.5,]; rownames(real_exports) <- 1:nrow(real_exports)
...
lapply(1:length(periodList), function(j){
  ...
  flows <- flows[order(flows$Median, decreasing = T),]; rownames(flows) <- 1:nrow(flows)
  ...
  s <- sankeyNetwork(Links = sankeyList$links, Nodes = sankeyList$nodes, Source = "Reporter",
                     Target = "Partner", Value = "Median", NodeID = "name",
                     units = "Ton", LinkGroup = "gType", fontSize = 12, nodeWidth = 30)
  saveNetwork(s, paste("D:/ToBackup/Modelling/global-futures-and-strategic-foresight/_graphics/_sankeyPlot-v5/sankey_", gsub(pattern = "-", replacement = "_", periodList[j]), ".html", sep = ""), selfcontained = T)
...
real_exports$Median <- log(real_exports$Median + 10, base=exp(1))
saveRDS(object = real_exports, file = './_data/_cassava_data/periods_median/log_dried_cassava_exports.rds')
...
lapply(1:length(periodList), function(i){
  ...
  # Save plot
  png(paste('./_graphics/static_plots/circos_', periodList[i],'.png', sep = ''), width = 10, height = 10, units = 'in', res = 300)
  
  chordDiagram(x = subReal[subReal$Median > 0, ], transparency = 0.25,
               directional = 1,
...
lapply(1:length(periodList), function(i){
  ...
  saveNetwork(chrd, paste("D:/ToBackup/Modelling/global-futures-and-strategic-foresight/_graphics/_interactive_chord_cassava_starch/version_2/chord_", gsub(pattern = "-", replacement = "_", periodList[i]), ".html", sep = ""), selfcontained = T)
