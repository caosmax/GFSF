...
kk<- unlist(xx)
kk<- kk[grep(pattern="_RCP2.6", x = kk)]
# kk<- list(kk)
...
for(i in seq_along(datatotal)){
      ...
      bb<- bb[c("impactparameter","commodity","region","RCP2.6_SSP2","RCP2.6_SSP4","RCP2.6_SSP5")]
      bb$"dp_2.6SSP4"<- ((bb$RCP2.6_SSP4-bb$RCP2.6_SSP2)/bb$RCP2.6_SSP2)*100
      bb$"dp_2.6SSP5"<- ((bb$RCP2.6_SSP5-bb$RCP2.6_SSP2)/bb$RCP2.6_SSP2)*100
      
      bb<- bb[c("impactparameter","commodity","region", "dp_2.6SSP4","dp_2.6SSP5"   )]
      ...
      ggsave(file= paste("test/",datatotal[i],"__SCEWB2.6.png",sep=""),e,width = 8,height = 8)
      write.csv(bb,paste("./tables/",datatotal[i],"__SCEWB2.6.csv",sep=""))
...
write.csv(brief,"./tables/summary_datatotal_SCEWB2.6.csv")
...
for(i in seq_along(dataagg)){
      ...
      ggsave(file= paste("test/",dataagg[i],"__SCEWB2.6.png",sep=""),e,width = 8,height = 8)
      write.csv(bb,paste("./tables/",dataagg[i],"__SCEWB2.6.csv",sep=""))
...
brief<- do.call(rbind,r)
write.csv(brief,"./tables/summary_dataagg_SCEWB2.6.csv")
...
nn<-  which(bb$RCP2.6_SSP2<0  & bb$RCP2.6_SSP4<0 & bb$RCP2.6_SSP5<0) # net trade negativo  importador neto
pp<-  which(bb$RCP2.6_SSP2>0 & bb$RCP2.6_SSP4>0 & bb$RCP2.6_SSP5>0) # net trade positivo  exportador neto
...
bb$"i_2.6SSP4"<- ((bb$RCP2.6_SSP4/bb$RCP2.6_SSP2))
bb$"i_2.6SSP5"<- ((bb$RCP2.6_SSP5/bb$RCP2.6_SSP2))

bb$impacto[import] <- "Negative Net Trade"
bb$impacto[export] <- "Positive Net Trade"
bb$impacto[is.na(bb$impacto)]<- "Transition" 

write.csv(bb, "./tables/NetTrade_GEN_SCEWB2.6.csv")


bb<- bb[c("impactparameter","commodity","region","impacto",  "i_2.6SSP4","i_2.6SSP5" )]
...
trade<- ggplot(data=bb[which(bb$commodity!="Sugar"),],aes(x=commodity,y=i))  +      
      ...
      theme(axis.text.x = element_text(angle =0, hjust = 1))+
      coord_flip()+
      labs(y="Index by 2050\n(Base=RCP2.6_SSP2)",x="Cultivations",title="Net Trade")+
      labs(fill=labs3)+
...
ggsave(file= paste("test/Trade_SCEWB2.6.png",sep=""),trade,width = 9,height = 7)
write.csv(bb, "./tables/NetTrade_SCEWB2.6.csv")
...
ggsave(file= paste("test/AuFood_SCEWB2.6.png",sep=""),aufood,width = 9,height = 7)
bb<- bb %>% spread(var,val) %>% dplyr::select(commodity, region, year, esce, au) %>% spread(esce,au)

## logica de los valores 
nn<-  which(bb$RCP2.6_SSP2<1 & bb$RCP2.6_SSP4<1 & bb$RCP2.6_SSP5<1) # net trade negativo  importador neto
pp<-  which(bb$RCP2.6_SSP2>1 & bb$RCP2.6_SSP4>1 & bb$RCP2.6_SSP5>1) # net trade positivo  exportador neto
...
write.csv(bb, "./tables/AuFood_SCEWB2.6.csv")
...
ggsave(file= paste("test/AuFood_Total_SCEWB2.6.png",sep=""),aufood,width = 9,height = 7)
