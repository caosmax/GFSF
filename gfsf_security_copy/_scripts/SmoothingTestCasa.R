...
##    World    ####
###################
WorldRice<- rice %>% filter(!Area %in% potsOut) %>% filter(Area!="Syrian Arab Republic")
...
tryCatch(lapply(1:length(pots), function(c){
      ...
      test_i<- ts(cfiles[5], start =c(1961))
      plot(test_i)
...
#       
      ...
      write.csv(db,file = paste("./RicePaper/temporal/", pots[[c]], "OutPutHWFAO.csv", sep = ""))
      ...
      test_a<- ts(cfiles[5], start =c(1961,1))
      plot.ts(test_a)
      ...
      write.csv(db_smooth,file = paste("./RicePaper/temporal/", pots[[c]], "OutPutSmoothFAO.csv", sep = ""))
      ...
      write.csv(brief,file = paste("./RicePaper/temporal/", pots[[c]], "briefDesy.csv", sep = ""))
...
### load data 
shockW<- list.files(path="./RicePaper/temporal", pattern = "briefDesy", all.files = T,full.names = T)
...
lapply(1:length(pots), function(c){
      ...
      write.csv(local,file = paste("./RicePaper/temporal/", pots[[c]], "_shocksLocal&ForeighFAO.csv", sep = ""))
...
### load data 
shockComplete<- list.files(path="./RicePaper/temporal/", pattern = "\\_shocksLocal&Foreigh", all.files = T,full.names = T)
...
for(v in 1:length(var)){
      ...
      for(x in 1:length(yr)){
            ...
            b<- mean(tt[,3])
            z<- (tt[,3]-b)/a
#             nn<- which(abs(tt[,3]-b)<a) # negativo
#             pp<- which(abs(tt[,3]-b)>a) # positivo
#             mr<- which(abs(tt[,3]))
#             which(gdata[,var[v]]<=-1)
#             abs(tt[,3]-b)<a
...
write.csv(gdata,file = paste("./RicePaper/", "NumericCategoriesFAO.csv", sep = ""))
...
hv<- prop.table(t1)
write.csv(hv,file = paste("./RicePaper/temporal/","Years_hv_shocksLocal&ForeighFAO.csv", sep = ""))
...
sma_t<- prop.table(t1)
write.csv(sma_t,file = paste("./RicePaper/temporal/","Years_sma_shocksLocal&ForeighFAO.csv", sep = ""))
...
ema_t<- prop.table(t1)
write.csv(sma_t,file = paste("./RicePaper/temporal/","Years_ema_shocksLocal&ForeighFAO.csv", sep = ""))
...
hv<- prop.table(t1)
write.csv(hv,file = paste("./RicePaper/temporal/","Years_lac_hv_shocksLocal&ForeighFAO.csv", sep = ""))
...
sma_t<- prop.table(t1)
write.csv(sma_t,file = paste("./RicePaper/temporal/","Years_lac_sma_shocksLocal&ForeighFAO.csv", sep = ""))
...
ema_t<- prop.table(t1)
write.csv(sma_t,file = paste("./RicePaper/temporal/","Years_lac_ema_shocksLocal&ForeighFAO.csv", sep = ""))


###################################### 

write.csv(sdata,file = paste("./RicePaper/temporal/","Categories_shocksLocal&ForeighFAO.csv", sep = ""))
