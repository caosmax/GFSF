...
md<-read.csv("CSA_Malawi.csv",header=T)
phi<- md
...
cropsout<- c("AMT", "AOT", "CER", "PUL", "SGR", "SGC", "MLS", "COT", "OLS", "OIL", "R&T", "F&V" )
phi<- phi %>% filter(., !commodity %in% cropsout)
...
require(xlsx)
write.xlsx( x = z,file= paste(copy, "MAL.xlsx", sep = ""),
...
write.csv(mdwide_tan,paste(copy,"mdwideMALTotal.csv", sep = ""), row.names = FALSE)
...
write.csv(tznet,paste(copy,"CambiosRelativosMALTOTAL.csv", sep = ""), row.names = FALSE)
...
write.csv(c, paste(copy, "NettradeMALTOTAL.csv", sep = ""),row.names = FALSE)
...
write.csv(x =tznet2_t, file =paste(copy,"areayieldMAL.csv",sep = ""))
...
write.csv(csa, paste(copy, "MALSAAreaYield.csv", sep = ""), row.names = FALSE)


#grafico Area de cultivo
png(filename=paste(grd,"AreaCommoditiesMAL.png", sep=""), width = 10, height = 10, units = 'in', res =800)
...
png(filename=paste(grd,"Numer Animales MAL.png", sep=""), width = 10, height = 10, units = 'in', res =800)
