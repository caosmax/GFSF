...
suppressMessages(if(!require(caTools)){install.packages("doSNOW");library(doSNOW)}else{library(doSNOW)})


library(parallel)
library(tidyverse)
library("compiler", lib.loc="~/R/R-3.4.1/library")
...
#     require(dplyr)
    ...
    tmax$Planting[which(tmax$Planting == -99)] <- NA
    tmax$Planting[which(tmax$Planting == 0)] <- NA
    tmin$Planting[which(tmin$Planting == -99)] <- NA
    tmin$Planting[which(tmin$Planting == 0)] <- NA
    tmean$Planting[which(tmean$Planting == -99)] <- NA
    tmean$Planting[which(tmean$Planting == 0)] <- NA
    ...
    tmax$Harvest <- round(tmax$Harvest)
    tmax$Planting <- round(tmax$Planting)
    
    tmin$Harvest <- round(tmin$Harvest)
    tmin$Planting <- round(tmin$Planting)
    
    tmean$Harvest <- round(tmean$Harvest)
    tmean$Planting <- round(tmean$Planting)
