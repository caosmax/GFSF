...
kk<- unlist(xx)
kk<- kk[grep(pattern="_RCP4.5", x = kk)]
# kk<- list(kk)
...
for(i in seq_along(datatotal)){
      ...
      bb<- bb[c("impactparameter","commodity","region","RCP4.5_SSP1", "RCP4.5_SSP2",
                "RCP4.5_SSP3","RCP4.5_SSP4", "RCP4.5_SSP5")]
      bb$"dp_4.5SSP1"<- ((bb$RCP4.5_SSP1-bb$RCP4.5_SSP2)/bb$RCP4.5_SSP2)*100
      bb$"dp_4.5SSP3"<- ((bb$RCP4.5_SSP3-bb$RCP4.5_SSP2)/bb$RCP4.5_SSP2)*100
      bb$"dp_4.5SSP4"<- ((bb$RCP4.5_SSP4-bb$RCP4.5_SSP2)/bb$RCP4.5_SSP2)*100
      bb$"dp_4.5SSP5"<- ((bb$RCP4.5_SSP5-bb$RCP4.5_SSP2)/bb$RCP4.5_SSP2)*100
      
      bb<- bb[c("impactparameter","commodity","region", "dp_4.5SSP1",
                "dp_4.5SSP3", "dp_4.5SSP4", "dp_4.5SSP5" )]
      ...
      ggsave(file= paste("test/",datatotal[i],"__Sce2_Sce1.png",sep=""),e,width = 8,height = 8)
      
      write.csv(bb,paste("./tables/",datatotal[i],"__Sce2_Sce1.csv",sep=""))
...
write.csv(brief,"./tables/summary_datatotal__Sce2_Sce1.csv")
...
for(i in seq_along(dataagg)){
      ...
      ggsave(file= paste("test/",dataagg[i],"__Sce2_Sce1.png",sep=""),e,width = 8,height = 8)
      write.csv(bb,paste("./tables/",dataagg[i],"__Sce2_Sce1.csv",sep=""))
...
brief<- do.call(rbind,r)
write.csv(brief,"./tables/summary_dataagg__Sce2_Sce1.csv")
...
nn<-  which(bb$RCP4.5_SSP1<0 & bb$RCP4.5_SSP2<0 & bb$RCP4.5_SSP3<0 & 
                  bb$RCP4.5_SSP4<0 & 
                  bb$RCP4.5_SSP5<0) # net trade negativo  importador neto
pp<-  which(bb$RCP4.5_SSP1>0 & bb$RCP4.5_SSP2>0 & bb$RCP4.5_SSP3>0 & 
                  bb$RCP4.5_SSP4>0 & 
                  bb$RCP4.5_SSP5>0) # net trade positivo  exportador neto
...
bb$"i_4.5SSP1"<- ((bb$RCP4.5_SSP1/bb$RCP4.5_SSP2))
bb$"i_4.5SSP3"<- ((bb$RCP4.5_SSP3/bb$RCP4.5_SSP2))
bb$"i_4.5SSP4"<- ((bb$RCP4.5_SSP4/bb$RCP4.5_SSP2))
bb$"i_4.5SSP5"<- ((bb$RCP4.5_SSP5/bb$RCP4.5_SSP2))

bb$impacto[import] <- "Negative Net Trade"
bb$impacto[export] <- "Positive Net Trade"
bb$impacto[is.na(bb$impacto)]<- "Transition" 

write.csv(bb, "./tables/NetTrade_GEN_Sce2_Sce1.csv")

bb<- bb[c("impactparameter","commodity","region","impacto", "i_4.5SSP1", "i_4.5SSP3", "i_4.5SSP4","i_4.5SSP5")]
...
trade<- ggplot(data=bb[which(bb$commodity!="Sugar"),],aes(x=commodity,y=i))  +      
      ...
      theme(axis.text.x = element_text(angle =0, hjust = 1))+
      coord_flip()+
      labs(y="Index by 2050\n(Base=RCP4.5_SSP2)",x="Cultivations",title="Net Trade")+
      labs(fill=labs3)+
...
ggsave(file= paste("test/Trade_Sce2_Sce1.png",sep=""),trade,width = 9,height = 7)
write.csv(bb, "./tables/NetTrade_Sce2_Sce1.csv")
...
ggsave(file= paste("test/AuFood_Sce2_Sce1.png",sep=""),aufood,width = 9,height = 7)
bb<- bb %>% spread(var,val) %>% dplyr::select(commodity, region, year, esce, au) %>% spread(esce,au)

## logica de los valores 
nn<-  which(bb$RCP4.5_SSP1<1 & bb$RCP4.5_SSP2<1 & bb$RCP4.5_SSP3<1 & bb$RCP4.5_SSP4<1 & bb$RCP4.5_SSP5<1) # net trade negativo  importador neto
pp<-  which(bb$RCP4.5_SSP1>1 & bb$RCP4.5_SSP2>1 & bb$RCP4.5_SSP3>1 & bb$RCP4.5_SSP4>1 & bb$RCP4.5_SSP5>1) # net trade positivo  exportador neto
...
write.csv(bb, "./tables/AuFood_Sce2_Sce1.csv")
...
ggsave(file= paste("test/AuFood_Total_Sce2_Sce1.png",sep=""),aufood,width = 9,height = 7)
