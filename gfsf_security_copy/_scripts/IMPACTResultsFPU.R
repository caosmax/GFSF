...
cfiles<- list.files(grd,pattern = "X0",full.names = T)
...
tc_all<- read.csv("//dapadfs/workspace_cluster_6/Socioeconomia/GF_and_SF/BID_2/GraphGrownRate/tc_comparison.csv")
tc_all$X<- NULL
...
Ymean<- read.csv(paste(grd1, "PromediosresumenPCA.csv", sep = ""))
Ymean$X<- NULL
...
out<- read.csv(paste(grd2, "SummaryOutliers.csv", sep = ""))
out$X<- NULL
...
saveRDS(object = cfiles,file =paste(grd3,"AnalysisBID.RDS", sep = ""))
