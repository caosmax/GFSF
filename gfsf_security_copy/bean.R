...
setwd("C:/Users/JJRODRIGUEZ/Documents/IMPACT Training 3.3/IMPACT3-Model-ver3.3/OutputFiles/Aggregation/")
#Dirrectorio de graficos
grd<-"C:/Users/JJRODRIGUEZ/Documents/GFSF/"

#Cargar lista de nombres de los archivos
nombres <- list.files(pattern ="*.csv" )
#Crear md con los nombre de los marco de datos
...
#ej<-droplevels(subset(ej,ej$Variable!="Yield"))
ejtec<-ej
...
tiff(filename=paste(grd,"dendograma.tiff",sep=""), 
...
mtscaled <- as.matrix(scale(ejtec_s))
...
bb<-heatmap(mtscaled[cutree(hc.rows,k=2)==1,], Colv=as.dendrogram(hc.cols), scale='none',
...
heatmap(mtscaled[cutree(hc.rows,k=1)==2,], Colv=as.dendrogram(hc.cols), scale='none')


tiff(filename=paste(grd,"Rplot.tiff",sep=""), 
     width = 15, height = 10, units = 'in', res = 100)
plot(hclust((dist(ejtec_s$Yield))),main=NULL,xlab="",ylab=NULL,sub="")
dev.off()

gg<-ejtec_s[complete.cases(ejtec_s),]


write.csv(ejtec_s,file="C:/Users/JJRODRIGUEZ/Documents/GFSF/cambios_oferta.csv")
