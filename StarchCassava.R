...
fao_matrix_trade<- read.csv("C:/Users/CEGONZALEZ/Documents/cassava/FAOSTAT/FAOMatrixStarch.csv", header = TRUE)
...
fao_matrix_trade<- fao_matrix_trade[which(fao_matrix_trade$Item!="Cassava dried"),]
fao_matrix_trade<- fao_matrix_trade[which(fao_matrix_trade$variable!="Import Quantity"),]
rownames(fao_matrix_trade)<- 1:nrow(fao_matrix_trade)


# copia de la base de datos en excel  
write.csv(fao_matrix_trade, paste(copy, "XmatrixtradecassavaStarch.csv", sep = ""), row.names=T)
fao_matrix_trade<- read.csv(paste(copy, "XmatrixtradecassavaStarch.csv", sep = ""))
...
png('p1wideStar.png', width=8, height=8, units='in', res=300)
...
png('p2wideStar.png', width=8, height=8, units='in', res=300)
...
png('p3wideStar.png', width=8, height=8, units='in', res=300)
...
png('p4wideStar.png', width=8, height=8, units='in', res=300)
...
write.csv(p1wide, paste(copy, "periodo1Star.csv", sep = ""), row.names=T)
write.csv(p2wide, paste(copy, "periodo2Star.csv", sep = ""), row.names=T)
write.csv(p3wide, paste(copy, "periodo3Star.csv", sep = ""), row.names=T)
write.csv(p4wide, paste(copy, "periodo4Star.csv", sep = ""), row.names=T)
